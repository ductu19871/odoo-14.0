...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        if not self._validate_group_account():
    ...
    def _validate_group_account(self):
        if not self.env.su and not (
                self.env.user.has_group('account.group_account_invoice') or
                self.env.user.has_group('sv_account.group_account_payment') or
                self.env.user.has_group('sv_account.group_account_billing_invoice')
        ):
            return False
        return True
