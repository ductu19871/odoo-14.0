...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _create_payments(self):
        ...
        if current_user.has_group('sv_account.group_account_payment'):
            payments = self.env['account.payment'].sudo().with_context(
                by_pass_group_payment=True
            ).create(payment_vals_list) # by pass group account payment
        else:
